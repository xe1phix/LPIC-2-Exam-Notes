...
■ 
													...
													## --------------------------------------------- ##
chsh −−list−shells						## List the Available shells
													## --------------------------------------------- ##
echo $SHELL								## Default shell used
													## --------------------------------------------- ##
chsh -s /bin/zsh xe1phix				## Set the default shell
													## --------------------------------------------- ##
...
echo "##-=================================================-##"
echo " 	  [+] Read In each line and do a DNS lookup			 "
echo "##-=================================================-##"
...
tunneling increases overhead, 
because it needs an extra set of IP headers. 
Typically this is 20 bytes per packet, 
so if the normal packet size (MTU) on a network is 1500
...
tune2fs - set the maximum mount count 
				before the system will automatically run fsck:
...
xfsrestore - xfsdump was executed with a block size of 4M. 
					which option do you need to invoke on xfsrestore 
					In order for it to use this dump successfully?
...
What ICMP message will be sent back to a traceroute client 
by every system along the path to the destination specified?
