...
A, AAAA, CAA, CNAME, MX, NS, TXT, and SRV records




Tier 1 servers, 
are the core DNS servers hosting authoritative zones, 
for all OpenNIC TLDs and the root (.) zone. 

Tier 2 servers 
are “DNS Resolvers” 
or, servers that actually do the heavy-lifting 
when querying OpenNICs DNS infrastructure. 
These are the servers that clients use in their configuration directly, 
...
PTR Record
    ...
    The PTR (Pointer) Record - configures reverse lookups for DNS (Reverse DNS), 
	such as for resolving IP addresses to hostnames. 
	PTR Records work in conjunction with A records 
	as they do the reverse work of each other.

    In LAB02, we connected a PTR record between the firewall machine (fw01) 
	and the Active Directory so that the AD and the workstation would be able 
	to ping the firewall from the IP address and have the hostname be returned.
